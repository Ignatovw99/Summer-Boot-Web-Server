...
public class ApplicationLoadingService {
    ...
    private void loadSolet(Class soletClass, String applicationName) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException {
        ...
        Object webSoletAnnotation = Arrays.stream(soletClass.getAnnotations())
                .filter(annotation -> annotation.annotationType().getSimpleName().equals(WebSolet.class.getSimpleName()))
                .findFirst()
                .orElse(null);

        String soletRoute = webSoletAnnotation.getClass()
                .getMethod("route")
                .invoke(webSoletAnnotation)
                .toString();

        if (!applicationName.equals("ROOT")) {
            soletRoute = "/".concat(applicationName).concat(soletRoute);
        }
        ...
        HttpSolet httpSoletProxy = (HttpSolet) Proxy.newProxyInstance(
                ...
                ((proxy, method, args) -> {
                    ...
                    if (extractedMethod.getName().equals("service")) {
                        ...
                        soletConfigObject.addAttribute("application-folder", this.applicationFolderPath);
    ...
    private void loadApplicationClasses(String classesRootFolderPath, String applicationName) throws IOException, ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        this.libraryClassUrls.add(new URL("file:/".concat(classesRootDirectory.getCanonicalPath()).concat(File.separator)));
    ...
    private void loadLibraryFile(JarFile library, String canonicalPath, String applicationName) throws MalformedURLException, ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        Enumeration<JarEntry> jarLibraryEntries = library.entries();

        while (jarLibraryEntries.hasMoreElements()) {
            JarEntry currentEntry = jarLibraryEntries.nextElement();

            if (!currentEntry.isDirectory() && currentEntry.getRealName().endsWith(".class")) {
                System.out.println("Loaded library: " + currentEntry);

                URL[] urls = new URL[] { new URL("jar:file:".concat(canonicalPath).concat("!/")) };
                URLClassLoader classLoader = new URLClassLoader(urls, Thread.currentThread().getContextClassLoader());
                Thread.currentThread().setContextClassLoader(classLoader);
                ...
                Class currentClassName = classLoader.loadClass(className);
                this.loadSolet(currentClassName, applicationName);
    ...
    private void loadApplicationLibraries(String librariesRootFolderPath, String applicationName) throws IOException, ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
        ...
        List<File> allJarLibraries = Arrays.stream(libraryFolder.listFiles())
                .filter(this::isJarFile)
                .collect(Collectors.toList());

        for (File jarLibrary : allJarLibraries) {
            if (jarLibrary != null) {
                JarFile libraryAsJar = new JarFile(jarLibrary.getCanonicalPath());
                this.loadLibraryFile(libraryAsJar, jarLibrary.getCanonicalPath(), applicationName);
            }
        }
    }
    ...
    private void loadApplicationFromFolder(String applicationRootFolderPath, String applicationName) throws IOException, ClassNotFoundException, InvocationTargetException, NoSuchMethodException, InstantiationException, IllegalAccessException {
        ...
        String classesRootFolderPath = applicationRootFolderPath
                .concat("classes")
                .concat(File.separator);

        String librariesRootFolderPath = applicationRootFolderPath
                .concat("lib")
                .concat(File.separator);
    ...
    public Map<String, HttpSolet> loadApplications(String applicationsFolderPath) throws IOException {
        ...
        try {
            ...
            if (applicationsFolder.exists() && applicationsFolder.isDirectory()) {
                List<File> applicationJarFiles = Arrays.stream(applicationsFolder.listFiles())
                        .filter(this::isJarFile)
                        .collect(Collectors.toList());

                for (File applicationJarFile : applicationJarFiles) {
                    this.jarFileUnzipService.unzipJar(applicationJarFile);

                    this.loadApplicationFromFolder(
                            applicationJarFile.getCanonicalPath().replace(".jar", File.separator),
